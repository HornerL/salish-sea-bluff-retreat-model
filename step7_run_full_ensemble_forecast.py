...
from config import WORKING_DIR, WORKING_DICTS, SLR, dfN_i, ERA5_WAVES, TRANSECT_DATA
...
# linear interp of MV model 
yrs = [2017, 2020, 2040, 2060, 2080, 2100] # each year that we will calculate a rate for

## linear interpolation of MV regression rates 000 and xxx slr
mv_2017 = df_mvar[['p_Rate','pi_lower','pi_upper']].values
mv_2100 = df_mvar2[['Rate','pi_lower','pi_upper']].values
mv_interp = {}
for i, yr in enumerate(yrs):
    f = (yr - 2017) / (2100 - 2017)  # interpolation fraction
    mv_interp[yr] = mv_2017 + f * (mv_2100 - mv_2017)

swashes = []
setups = []

k=0
for k in range (len(cSLR)):

    dfN_Fcast = 'AllForecastRates_IslandCo_' + str(cSLR[k]) + 'cm_by2100_Test_w_MV'
    dfN_Fcast2 = 'JustEnsembleRates_IslandCo_' + str(cSLR[k]) + 'cm_by2100_Test_w_MV'
    
    cSL = cSLR[k] / 100 # sea level at 2100, in meters
